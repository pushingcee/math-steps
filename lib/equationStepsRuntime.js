/* =====================================================================
 * EQUATION STEPS RUNTIME
 * Framework-agnostic mount of the step engine + animated renderer.
 * Originally renderer.js (a single global script driven by document ids);
 * refactored here into mountEquationSteps(root, states) so it can be
 * driven by a React component and instantiated more than once per page.
 *
 ...
 * =================================================================== */
import { plan, RONE, rabs, relStr, req, risInt, rnegq } from './engine';

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

/* ---------------------------------------------------------------------
 * Pure tile/HTML builders (depend only on the engine math helpers and
 * their arguments — safe to share across every mounted instance).
 * ------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------
 * mountEquationSteps(root, states)
 * Builds the lesson UI inside `root`, plays the steps for the supplied
 * ordering of states, and returns a controller with destroy().
 * ------------------------------------------------------------------- */
export function mountEquationSteps(root, states){
  root.innerHTML = `
    <div class="es-error-box"></div>
    <div class="stage-wrap">
      <div class="stage">
        <div class="op-badge"></div>
        <svg class="arrow-layer"><path class="arrow-path" d=""></path><polygon class="arrow-head" points=""></polygon></svg>
        <div class="equation"></div>
      </div>
    </div>
    <div class="caption"><span class="badge">Start</span><span class="caption-text"></span></div>
    <div class="controls">
      <button type="button" class="es-prev">&#8592; Back</button>
      <div class="dots"></div>
      <button type="button" class="es-next primary">Next &#8594;</button>
      <button type="button" class="es-replay">&#8635; Replay</button>
    </div>`;

  const stage=root.querySelector('.stage');
  const eqEl=root.querySelector('.equation');
  const opBadge=root.querySelector('.op-badge');
  const arrowLayer=root.querySelector('.arrow-layer');
  const arrowPath=root.querySelector('.arrow-path');
  const arrowHead=root.querySelector('.arrow-head');
  const captionEl=root.querySelector('.caption');
  const captionBadge=root.querySelector('.caption .badge');
  const captionText=root.querySelector('.caption-text');
  const dotsEl=root.querySelector('.dots');
  const btnPrev=root.querySelector('.es-prev');
  const btnNext=root.querySelector('.es-next');
  const btnReplay=root.querySelector('.es-replay');
  const errorBox=root.querySelector('.es-error-box');

  let RT=eqEl;        /* active render target: eqEl, or a branch column */
  let branchCols=[];  /* column containers when a branched state is shown */
  let lesson=null, stepIndex=-1, animating=false, destroyed=false;
  const flyers=[];    /* outstanding flyer overlays, removed on destroy */
  ...
  async function flyTile(termId,newState,duration=900){
    ...
    document.body.appendChild(flyer); flyers.push(flyer); source.style.visibility='hidden';
    ...
    flyer.remove(); const fi=flyers.indexOf(flyer); if(fi>=0) flyers.splice(fi,1);
  ...
  async function next(){ if(!lesson||animating||stepIndex>=lesson.steps.length-1) return; animating=true; updateButtons(); stepIndex++;
    ...
    await runStep(step); if(destroyed) return; animating=false; updateButtons(); }
  ...
  function build(){ RT=eqEl; errorBox.classList.remove('visible'); const lines=(states||[]).map(s=>String(s).trim()).filter(Boolean);
    try{ lesson=plan(lines); replay(); }
    catch(e){ lesson=null; eqEl.innerHTML=''; dotsEl.innerHTML=''; setCaption('Fix the states, then rebuild.','Error'); errorBox.innerHTML='<strong>Can’t build this lesson:</strong> '+e.message; errorBox.classList.add('visible'); updateButtons(); } }

  function onKey(e){ if(e.key==='ArrowRight'){ next(); } else if(e.key==='ArrowLeft'){ prev(); } }
  ...
  return {
    next, prev, replay, build,
    destroy(){
      destroyed=true;
      root.removeEventListener('keydown',onKey);
      flyers.forEach(f=>f.remove()); flyers.length=0;
      lesson=null; root.innerHTML='';
    }
  };
}
